...
import { TimeSeriesData, QueryParams, ApiResponse } from '../types/api';
...
const Dashboard = () => {
    ...
    // Prepare chart data
    const chartData = {
        labels: data.map((item: TimeSeriesData) => new Date(item.timestamp).toLocaleDateString()),
        datasets: [
            {
                label: `${metric} - ${userId}`,
    ...
    const chartOptions = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: `Average HR Data per Day - ${userId}`,
            },
        },
        scales: {
            y: {
